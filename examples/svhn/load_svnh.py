...
class Datum:
    ...
    def prepare_data(self):
        #conf_label = np.zeros((LABEL_CT, TOP_HEIGHT, TOP_WIDTH), dtype = np.float)
        #conf_label = np.zeros((LABEL_CT, TOP_HEIGHT, TOP_WIDTH), dtype = np.float)
            ...
            for j in range(TOP_WIDTH):
                for rect in self.rect_iter():
                    ...
                    if rect.intersects(obs_rect):
                        #conf_label[:, i, j] = np.argmax(self.label_array)
        ...
        #trans_img = np.transpose(self.image, (0, 1))
        trans_img = self.image.reshape(1, IMG_HEIGHT, IMG_WIDTH)

        #print conf_label[:,0,0]
        #conf_label.reshape(LABEL_CT, 1, IMG_HEIGHT, IMG_WIDTH)
        return (trans_img, bbox_label, conf_label)

    def display(self, rgb_color=(0,255,0)):
        # Draws rectangle on copy of image (greyscale or not).  
